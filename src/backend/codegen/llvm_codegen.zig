...
pub const LlvmCompiler = struct {
    ...
    pub fn generateWasmMemoryFunctions(self: *LlvmCompiler) !void {
        ...
        // kyte_str_alloc(len): allocate len+1 bytes via kyte_bytes_alloc, NUL-terminate at
        // [len], and rewrite the i32 ARC length header (@ ptr-4, which bytes_alloc set to
        // len+1) to the logical len. This mirrors the native runtime (core.cpp) so the wasm
        // string path (concat, interpolation) is self-contained (embed-wasm.md M0-a).
        {
            var sa_params = [_]types.LLVMTypeRef{self.val_type};
            const sa_type = core.LLVMFunctionType(self.val_type, &sa_params, 1, 0);
            const sa_fn = core.LLVMAddFunction(self.module, "kyte_str_alloc", sa_type);
            try self.func_map.put("kyte_str_alloc", sa_fn);

            const sa_bb = core.LLVMAppendBasicBlock(sa_fn, "entry");
            const sbld = core.LLVMCreateBuilder();
            defer core.LLVMDisposeBuilder(sbld);
            core.LLVMPositionBuilderAtEnd(sbld, sa_bb);

            const sa_len = core.LLVMGetParam(sa_fn, 0);
            const sa_size = core.LLVMBuildAdd(sbld, sa_len, core.LLVMConstInt(self.val_type, 1, 0), "sa_size");
            var sa_args = [_]types.LLVMValueRef{sa_size};
            const sa_ptr = core.LLVMBuildCall2(sbld, alloc_type, alloc_fn, &sa_args, 1, "sa_buf");

            const sa_hdr_int = core.LLVMBuildSub(sbld, sa_ptr, core.LLVMConstInt(self.val_type, 4, 0), "sa_hdr");
            const sa_hdr_ptr = core.LLVMBuildIntToPtr(sbld, sa_hdr_int, core.LLVMPointerType(self.i32_type, 0), "sa_hdrp");
            _ = core.LLVMBuildStore(sbld, core.LLVMBuildTrunc(sbld, sa_len, self.i32_type, "sa_len32"), sa_hdr_ptr);

            const sa_term_int = core.LLVMBuildAdd(sbld, sa_ptr, sa_len, "sa_term");
            const sa_term_ptr = core.LLVMBuildIntToPtr(sbld, sa_term_int, core.LLVMPointerType(self.i8_type, 0), "sa_termp");
            _ = core.LLVMBuildStore(sbld, core.LLVMConstInt(self.i8_type, 0, 0), sa_term_ptr);

            _ = core.LLVMBuildRet(sbld, sa_ptr);
        }

        // kyte_bytes_copy(dst, src, len): a byte memcpy over linear memory, the wasm body for
        // the native runtime symbol that string concat and slicing use (embed-wasm.md M0-a).
        {
            var bc_params = [_]types.LLVMTypeRef{ self.val_type, self.val_type, self.val_type };
            const bc_type = core.LLVMFunctionType(self.void_type, &bc_params, 3, 0);
            const bc_fn = core.LLVMAddFunction(self.module, "kyte_bytes_copy", bc_type);
            try self.func_map.put("kyte_bytes_copy", bc_fn);

            const bc_entry = core.LLVMAppendBasicBlock(bc_fn, "entry");
            const bc_cond = core.LLVMAppendBasicBlock(bc_fn, "cond");
            const bc_body = core.LLVMAppendBasicBlock(bc_fn, "body");
            const bc_done = core.LLVMAppendBasicBlock(bc_fn, "done");
            const cb = core.LLVMCreateBuilder();
            defer core.LLVMDisposeBuilder(cb);

            core.LLVMPositionBuilderAtEnd(cb, bc_entry);
            const bc_dst = core.LLVMGetParam(bc_fn, 0);
            const bc_src = core.LLVMGetParam(bc_fn, 1);
            const bc_len = core.LLVMGetParam(bc_fn, 2);
            const bc_ctr = core.LLVMBuildAlloca(cb, self.val_type, "ctr");
            _ = core.LLVMBuildStore(cb, core.LLVMConstInt(self.val_type, 0, 0), bc_ctr);
            _ = core.LLVMBuildBr(cb, bc_cond);

            core.LLVMPositionBuilderAtEnd(cb, bc_cond);
            const bc_i = core.LLVMBuildLoad2(cb, self.val_type, bc_ctr, "i");
            const bc_cmp = core.LLVMBuildICmp(cb, .LLVMIntULT, bc_i, bc_len, "more");
            _ = core.LLVMBuildCondBr(cb, bc_cmp, bc_body, bc_done);

            core.LLVMPositionBuilderAtEnd(cb, bc_body);
            const bc_i2 = core.LLVMBuildLoad2(cb, self.val_type, bc_ctr, "i2");
            const bc_srcp = core.LLVMBuildIntToPtr(cb, core.LLVMBuildAdd(cb, bc_src, bc_i2, "sa"), core.LLVMPointerType(self.i8_type, 0), "srcp");
            const bc_byte = core.LLVMBuildLoad2(cb, self.i8_type, bc_srcp, "byte");
            const bc_dstp = core.LLVMBuildIntToPtr(cb, core.LLVMBuildAdd(cb, bc_dst, bc_i2, "da"), core.LLVMPointerType(self.i8_type, 0), "dstp");
            _ = core.LLVMBuildStore(cb, bc_byte, bc_dstp);
            _ = core.LLVMBuildStore(cb, core.LLVMBuildAdd(cb, bc_i2, core.LLVMConstInt(self.val_type, 1, 0), "inc"), bc_ctr);
            _ = core.LLVMBuildBr(cb, bc_cond);

            core.LLVMPositionBuilderAtEnd(cb, bc_done);
            _ = core.LLVMBuildRetVoid(cb);
        }

        // kyte_valopt_box(value): box = kyte_bytes_alloc(8); store value; return box.
        // kyte_valopt_unbox(box): box==0 ? 0 : load. Value-optional heap boxing, the wasm
        // bodies for the native primitives the optional/string paths use (embed-wasm.md M0-a).
        {
            var vb_params = [_]types.LLVMTypeRef{self.val_type};
            const vb_type = core.LLVMFunctionType(self.val_type, &vb_params, 1, 0);
            const vb_fn = core.LLVMAddFunction(self.module, "kyte_valopt_box", vb_type);
            try self.func_map.put("kyte_valopt_box", vb_fn);
            const vb_bb = core.LLVMAppendBasicBlock(vb_fn, "entry");
            const vbb = core.LLVMCreateBuilder();
            defer core.LLVMDisposeBuilder(vbb);
            core.LLVMPositionBuilderAtEnd(vbb, vb_bb);
            const vb_val = core.LLVMGetParam(vb_fn, 0);
            var vb_args = [_]types.LLVMValueRef{core.LLVMConstInt(self.val_type, 8, 0)};
            const vb_box = core.LLVMBuildCall2(vbb, alloc_type, alloc_fn, &vb_args, 1, "vbox");
            const vb_box_ptr = core.LLVMBuildIntToPtr(vbb, vb_box, core.LLVMPointerType(self.val_type, 0), "vboxp");
            _ = core.LLVMBuildStore(vbb, vb_val, vb_box_ptr);
            _ = core.LLVMBuildRet(vbb, vb_box);
        }
        {
            var vu_params = [_]types.LLVMTypeRef{self.val_type};
            const vu_type = core.LLVMFunctionType(self.val_type, &vu_params, 1, 0);
            const vu_fn = core.LLVMAddFunction(self.module, "kyte_valopt_unbox", vu_type);
            try self.func_map.put("kyte_valopt_unbox", vu_fn);
            const vu_entry = core.LLVMAppendBasicBlock(vu_fn, "entry");
            const vu_null = core.LLVMAppendBasicBlock(vu_fn, "isnull");
            const vu_load = core.LLVMAppendBasicBlock(vu_fn, "load");
            const vub = core.LLVMCreateBuilder();
            defer core.LLVMDisposeBuilder(vub);
            core.LLVMPositionBuilderAtEnd(vub, vu_entry);
            const vu_box = core.LLVMGetParam(vu_fn, 0);
            const vu_isz = core.LLVMBuildICmp(vub, .LLVMIntEQ, vu_box, core.LLVMConstInt(self.val_type, 0, 0), "isz");
            _ = core.LLVMBuildCondBr(vub, vu_isz, vu_null, vu_load);
            core.LLVMPositionBuilderAtEnd(vub, vu_null);
            _ = core.LLVMBuildRet(vub, core.LLVMConstInt(self.val_type, 0, 0));
            core.LLVMPositionBuilderAtEnd(vub, vu_load);
            const vu_ptr = core.LLVMBuildIntToPtr(vub, vu_box, core.LLVMPointerType(self.val_type, 0), "vup");
            const vu_v = core.LLVMBuildLoad2(vub, self.val_type, vu_ptr, "vuv");
            _ = core.LLVMBuildRet(vub, vu_v);
        }

        // kyte_bytes_alloc_persistent_nz(size): the non-zeroing persistent allocator. The wasm
        // bump heap does not zero on alloc anyway, so route it to kyte_bytes_alloc (M0-a).
        {
            var nz_params = [_]types.LLVMTypeRef{self.val_type};
            const nz_type = core.LLVMFunctionType(self.val_type, &nz_params, 1, 0);
            const nz_fn = core.LLVMAddFunction(self.module, "kyte_bytes_alloc_persistent_nz", nz_type);
            try self.func_map.put("kyte_bytes_alloc_persistent_nz", nz_fn);
            const nz_bb = core.LLVMAppendBasicBlock(nz_fn, "entry");
            const nzb = core.LLVMCreateBuilder();
            defer core.LLVMDisposeBuilder(nzb);
            core.LLVMPositionBuilderAtEnd(nzb, nz_bb);
            var nz_args = [_]types.LLVMValueRef{core.LLVMGetParam(nz_fn, 0)};
            const nz_r = core.LLVMBuildCall2(nzb, alloc_type, alloc_fn, &nz_args, 1, "nz");
            _ = core.LLVMBuildRet(nzb, nz_r);
        }
        ...
        // kyte_i64_to_string(v): a self-contained base-10 formatter. Writes digits backwards
        // into a scratch buffer, then allocates an exact-length string and copies. This gives
        // numbers a wasm body (no host import), which KYX interpolation needs (embed-wasm.md M0-a).
        {
            const sa_fn2 = self.func_map.get("kyte_str_alloc").?;
            var sa_p2 = [_]types.LLVMTypeRef{self.val_type};
            const sa_t2 = core.LLVMFunctionType(self.val_type, &sa_p2, 1, 0);
            const bc_fn2 = self.func_map.get("kyte_bytes_copy").?;
            var bc_p2 = [_]types.LLVMTypeRef{ self.val_type, self.val_type, self.val_type };
            const bc_t2 = core.LLVMFunctionType(self.void_type, &bc_p2, 3, 0);

            var i2s_params = [_]types.LLVMTypeRef{self.val_type};
            const i2s_type = core.LLVMFunctionType(self.val_type, &i2s_params, 1, 0);
            const i2s_fn = core.LLVMAddFunction(self.module, "kyte_i64_to_string", i2s_type);
            try self.func_map.put("kyte_i64_to_string", i2s_fn);

            const e_bb = core.LLVMAppendBasicBlock(i2s_fn, "entry");
            const loop_bb = core.LLVMAppendBasicBlock(i2s_fn, "loop");
            const after_bb = core.LLVMAppendBasicBlock(i2s_fn, "after");
            const neg_bb = core.LLVMAppendBasicBlock(i2s_fn, "neg");
            const fin_bb = core.LLVMAppendBasicBlock(i2s_fn, "fin");
            const ib = core.LLVMCreateBuilder();
            defer core.LLVMDisposeBuilder(ib);

            core.LLVMPositionBuilderAtEnd(ib, e_bb);
            const v = core.LLVMGetParam(i2s_fn, 0);
            var scr_args = [_]types.LLVMValueRef{core.LLVMConstInt(self.val_type, 32, 0)};
            const scratch = core.LLVMBuildCall2(ib, alloc_type, alloc_fn, &scr_args, 1, "scratch");
            const is_neg = core.LLVMBuildICmp(ib, .LLVMIntSLT, v, core.LLVMConstInt(self.val_type, 0, 0), "isneg");
            const negv = core.LLVMBuildSub(ib, core.LLVMConstInt(self.val_type, 0, 0), v, "negv");
            const uval0 = core.LLVMBuildSelect(ib, is_neg, negv, v, "u0");
            const uu = core.LLVMBuildAlloca(ib, self.val_type, "uu");
            _ = core.LLVMBuildStore(ib, uval0, uu);
            const pos = core.LLVMBuildAlloca(ib, self.val_type, "pos");
            _ = core.LLVMBuildStore(ib, core.LLVMConstInt(self.val_type, 31, 0), pos);
            _ = core.LLVMBuildBr(ib, loop_bb);

            core.LLVMPositionBuilderAtEnd(ib, loop_bb);
            const u = core.LLVMBuildLoad2(ib, self.val_type, uu, "u");
            const d = core.LLVMBuildURem(ib, u, core.LLVMConstInt(self.val_type, 10, 0), "d");
            const d8 = core.LLVMBuildTrunc(ib, d, self.i8_type, "d8");
            const ch = core.LLVMBuildAdd(ib, d8, core.LLVMConstInt(self.i8_type, '0', 0), "ch");
            const p = core.LLVMBuildLoad2(ib, self.val_type, pos, "p");
            const wp = core.LLVMBuildIntToPtr(ib, core.LLVMBuildAdd(ib, scratch, p, "wa"), core.LLVMPointerType(self.i8_type, 0), "wp");
            _ = core.LLVMBuildStore(ib, ch, wp);
            _ = core.LLVMBuildStore(ib, core.LLVMBuildSub(ib, p, core.LLVMConstInt(self.val_type, 1, 0), "pdec"), pos);
            const un = core.LLVMBuildUDiv(ib, u, core.LLVMConstInt(self.val_type, 10, 0), "un");
            _ = core.LLVMBuildStore(ib, un, uu);
            const more = core.LLVMBuildICmp(ib, .LLVMIntNE, un, core.LLVMConstInt(self.val_type, 0, 0), "more");
            _ = core.LLVMBuildCondBr(ib, more, loop_bb, after_bb);

            core.LLVMPositionBuilderAtEnd(ib, after_bb);
            _ = core.LLVMBuildCondBr(ib, is_neg, neg_bb, fin_bb);

            core.LLVMPositionBuilderAtEnd(ib, neg_bb);
            const pmn = core.LLVMBuildLoad2(ib, self.val_type, pos, "pmn");
            const wpm = core.LLVMBuildIntToPtr(ib, core.LLVMBuildAdd(ib, scratch, pmn, "wam"), core.LLVMPointerType(self.i8_type, 0), "wpm");
            _ = core.LLVMBuildStore(ib, core.LLVMConstInt(self.i8_type, '-', 0), wpm);
            _ = core.LLVMBuildStore(ib, core.LLVMBuildSub(ib, pmn, core.LLVMConstInt(self.val_type, 1, 0), "pmdec"), pos);
            _ = core.LLVMBuildBr(ib, fin_bb);

            core.LLVMPositionBuilderAtEnd(ib, fin_bb);
            const pf = core.LLVMBuildLoad2(ib, self.val_type, pos, "pf");
            const start = core.LLVMBuildAdd(ib, pf, core.LLVMConstInt(self.val_type, 1, 0), "start");
            const len = core.LLVMBuildSub(ib, core.LLVMConstInt(self.val_type, 32, 0), start, "len");
            var res_args = [_]types.LLVMValueRef{len};
            const res = core.LLVMBuildCall2(ib, sa_t2, sa_fn2, &res_args, 1, "res");
            const src = core.LLVMBuildAdd(ib, scratch, start, "src");
            var cp_args = [_]types.LLVMValueRef{ res, src, len };
            _ = core.LLVMBuildCall2(ib, bc_t2, bc_fn2, &cp_args, 3, "");
            _ = core.LLVMBuildRet(ib, res);
        }
    }
